...
def plot_traces(log, include=['TN1', 'TN2', 'CL1', 'TB1', 'CPU4', 'CPU1', 'motor'],
    ...
    color_dict = {'TL2': tl2_color, 'CL1': cl1_color}  # 修改变量名
        ...
        if cell_type in ['TN1', 'TN2']:
            ...
            if i % 2 == 0:
                l = ax[i].legend(handles=handles,
                                 ...
                                 handlelength=0,
                                 handletextpad=0)
                legend_colors = [flow_color_L, flow_color_R]  # 修改变量名
                for i, text in enumerate(l.get_texts()):
            ...
            ax[i].plot(data[cell_type][0], color=color_dict[cell_type]);  # 修改变量名
...
def plot_angular_distances(noise_levels, angular_distances, bins=18, ax=None,
    ...
    for i in reversed(list(range(len(noise_levels)))):
...
def plot_cxr_weights(cx, label_font_size=11, unit_font_size=10,
    ...
    ticklabels = {'TL2': list(range(1, 17)),
                  'CL1': list(range(1, 17)),
                  'TB1': list(range(1, 9)),
                  'TN': ['L', 'R'],
                  'CPU4': list(range(1, 17)),
                  'Pontin': list(range(1, 17)),
                  'CPU1a': list(range(2, 16)),
                  'CPU1b': list(range(8, 10))}
